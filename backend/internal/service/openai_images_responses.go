...
func (s *OpenAIGatewayService) collectOpenAIImagesOAuthNonStreamingBody(
	...
	if len(results) == 0 {
		...
		// 诊断摘要由 reportOpenAIImagesOAuthCollectError 记录到 ops。返回
...
// 不可重试的上游错误会直接写响应，可重试错误留给 handler failover。
func (s *OpenAIGatewayService) reportOpenAIImagesOAuthCollectError(c *gin.Context, body []byte, err error) {
	...
	detail := ""
	if upstreamErr.Code == "content_policy_violation" || upstreamErr.Code == "image_generation_unavailable" {
		detail = summarizeOpenAIImagesNoOutputBody(body)
	}
	setOpsUpstreamError(c, upstreamErr.clientStatusCode(), upstreamErr.clientMessage(), detail)
...
func (s *OpenAIGatewayService) runOpenAIImagesOAuthFanoutAttempt(
	...
	if err != nil {
		if shouldClassifyOpenAIUpstreamStreamReadError(err, attempt.req.Context()) {
			err = newOpenAIUpstreamStreamReadError(err)
		}
		result.err = err
		return result
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthFanoutProblem(
	...
	if problem.err != nil {
		...
		var upstreamErr *OpenAIImagesUpstreamError
		var failoverErr *UpstreamFailoverError
		_, _, streamReadErr := OpenAIUpstreamStreamReadErrorDetails(problem.err)
		if errors.As(problem.err, &upstreamErr) || errors.As(problem.err, &failoverErr) || streamReadErr {
			if upstreamErr != nil || failoverErr != nil {
