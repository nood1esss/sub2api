...
func TestOpenAIGatewayServiceForwardImages_OAuthEditsMultipartUsesResponsesAPI(t *testing.T) {
	...
	editResponseBody := func(result, revisedPrompt string) string {
		return "data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000002,\"usage\":{\"input_tokens\":13,\"output_tokens\":21,\"output_tokens_details\":{\"image_tokens\":8}},\"tool_usage\":{\"image_gen\":{\"images\":1}},\"output\":[{\"type\":\"image_generation_call\",\"result\":\"" +
			result + "\",\"revised_prompt\":\"" + revisedPrompt + "\",\"output_format\":\"webp\",\"quality\":\"high\"}]}}\n\n" +
			"data: [DONE]\n\n"
	}
	upstream := &httpUpstreamRecorder{
		delay: 50 * time.Millisecond,
		responses: []*http.Response{
			{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"},
					"X-Request-Id": []string{"req_img_edit_1"},
				},
				Body: io.NopCloser(strings.NewReader(editResponseBody("ZWRpdGVkLTE=", "replace background 1"))),
			},
			{
				StatusCode: http.StatusOK,
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"},
					"X-Request-Id": []string{"req_img_edit_2"},
				},
				Body: io.NopCloser(strings.NewReader(editResponseBody("ZWRpdGVkLTI=", "replace background 2"))),
			},
			{
				StatusCode: http.StatusOK,
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"},
					"X-Request-Id": []string{"req_img_edit_3"},
				},
				Body: io.NopCloser(strings.NewReader(editResponseBody("ZWRpdGVkLTM=", "replace background 3"))),
			},
			{
				StatusCode: http.StatusOK,
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"},
					"X-Request-Id": []string{"req_img_edit_4"},
				},
				Body: io.NopCloser(strings.NewReader(editResponseBody("ZWRpdGVkLTQ=", "replace background 4"))),
			},
		},
	}
	svc.httpUpstream = upstream
	...
	require.Equal(t, 84, result.Usage.OutputTokens)
	require.Equal(t, 32, result.Usage.ImageOutputTokens)
	...
	for _, upstreamBody := range upstream.bodies {
		...
		require.False(t, gjson.GetBytes(upstreamBody, "tools.0.n").Exists())
		require.False(t, gjson.GetBytes(upstreamBody, "tools.0.input_fidelity").Exists())
		require.Equal(t, "webp", gjson.GetBytes(upstreamBody, "tools.0.output_format").String())
		require.True(t, strings.HasPrefix(gjson.GetBytes(upstreamBody, "input.0.content.1.image_url").String(), "data:image/png;base64,"))
		require.True(t, strings.HasPrefix(gjson.GetBytes(upstreamBody, "tools.0.input_image_mask.image_url").String(), "data:image/png;base64,"))
		require.Equal(t, "replace background with aurora", gjson.GetBytes(upstreamBody, "input.0.content.0.text").String())
	}

	data := gjson.Get(rec.Body.String(), "data").Array()
	require.Len(t, data, 4)
	require.ElementsMatch(t, []string{"ZWRpdGVkLTE=", "ZWRpdGVkLTI=", "ZWRpdGVkLTM=", "ZWRpdGVkLTQ="}, []string{data[0].Get("b64_json").String(), data[1].Get("b64_json").String(), data[2].Get("b64_json").String(), data[3].Get("b64_json").String()})
	require.ElementsMatch(t, []string{"replace background 1", "replace background 2", "replace background 3", "replace background 4"}, []string{data[0].Get("revised_prompt").String(), data[1].Get("revised_prompt").String(), data[2].Get("revised_prompt").String(), data[3].Get("revised_prompt").String()})
}
